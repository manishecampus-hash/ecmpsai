      ...
      <div className="max-w-7xl mx-auto text-center font-[Inter]">
        <div className="mb-8 sm:mb-10">
          ...
          <h2 className="mt-2 text-[23px] font-bold tracking-tight text-gray-900 sm:text-3xl md:text-4xl">
            Enhance Skills by <span className="text-red-500">Domain</span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-5 lg:gap-6 text-left">
          {careerExplorer.map((item, index) => (
            <div
              key={index}
              className="group relative bg-white rounded-2xl border border-gray-100 shadow-sm p-3 sm:p-4 flex flex-col transition-shadow duration-300 hover:shadow-md overflow-hidden cursor-pointer"
            >
              {/* Image Container - size same rahega */}
              ...
              >
                <div className="flex flex-col gap-1.5 sm:gap-2">
                  ...
                  <p className="text-white/90 text-[10px] sm:text-[11px] leading-snug line-clamp-5">
                    {item.description ??
                      "Explore top-ranked programs, career paths and industry insights in this field. Discover trending specializations, admission trends and what recruiters look for."}
                  </p>
                  ...
                  {item.highlights && item.highlights.length > 0 && (
                    <ul className="flex flex-col gap-1 mt-1">
                      {item.highlights.slice(0, 3).map((point, i) => (
                        <li
                          key={i}
                          className="flex items-center gap-1.5 text-white/80 text-[9px] sm:text-[10px]"
                        >
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    type="button"
                    className="inline-flex items-center gap-1 text-[10px] sm:text-[11px] font-bold text-white
