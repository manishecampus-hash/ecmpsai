        ...
        return (
          <React.Fragment key={step.id}>
            ...
            >
              ...
              <span
                className={`text-[10px] font-medium hidden sm:block ${idx <= current ? "text-gray-700" : "text-gray-400"}`}
              >
                {step.label}
              </span>
            </div>
            {idx < STEPS.length - 1 && (
              <div
                className={`h-0.5 w-5 mx-0.5 mb-3 transition-colors ${idx < current ? "bg-emerald-400" : "bg-gray-200"}`}
              />
            )}
          </React.Fragment>
        );
