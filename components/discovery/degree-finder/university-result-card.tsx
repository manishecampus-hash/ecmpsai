...
export const mbaUniversities: University[] = [
    ...
    },
    {
        ...
        logoText: 'AU',
        logoColor: '#8B1A1A',
        accreditations: ['UGC-DEB', 'AICTE', 'NIRF', 'WES', 'QS World University Rankings', 'DEC'],
        reviews: 1755,
        rating: 4.7,
        highlights: [
    ...
    return (
        <div className="flex items-center gap-1">
            {[1, 2, 3, 4, 5].map(i => {
                ...
                return (
                    <span key={i} className="text-base leading-none" style={{ color: filled || half ? '#F5A623' : '#D1D5DB' }}>
                        {filled ? '★' : half ? '⯨' : '☆'}
                    </span>
                );
            })}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
            ...
            <div className="flex items-start justify-between gap-4 p-5 pb-4">
                ...
                <div className="flex items-start gap-4 flex-1 min-w-0">
                    <div className="shrink-0">
                        <LogoAvatar uni={uni} />
                        <p className="text-[10px] text-gray-400 text-center mt-1 max-w-[64px] leading-tight">{uni.name.split(' ').slice(0, 2).join(' ')}</p>
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-[10px] text-gray-500 font-medium mb-1">Accredited by:</p>
                ...
                <div className="shrink-0 text-center">
                    <p className="text-xs text-gray-500 mb-1">{uni.reviews.toLocaleString()} Reviews</p>
                    <p className="text-2xl font-extrabold text-gray-900 leading-none">{uni.rating.toFixed(1)}</p>
            ...
            <div className="mx-5 mb-4 bg-indigo-50 rounded-xl px-4 py-2.5 flex flex-wrap gap-x-6 gap-y-1">
                {uni.highlights.map((h, i) => (
                    <span key={i} className="flex items-center gap-1.5 text-xs text-indigo-800">
            ...
            <div className="flex flex-wrap items-center gap-3 px-5 pb-5">
                ...
                <label className="flex items-center gap-1.5 cursor-pointer">
                    ...
                    <span className="text-xs text-gray-600 font-medium">Compare University</span>
                </label>
                ...
                >
                    <span className="bg-blue-700 hover:bg-blue-800 transition-colors text-white text-sm font-bold px-4 py-2.5 flex flex-col leading-tight">
                        ...
                        {uni.savings && (
                            <span className="text-[10px] font-normal text-emerald-300">You save ₹ {uni.savings.toLocaleString('en-IN')} *</span>
                        )}
                    </span>
