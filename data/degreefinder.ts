  ...
  return courses.filter((course) => {
    ...
    if (answers.budget) {
      ...
      if (opt && course.feeMin !== undefined && course.feeMax !== undefined) {
        if (
          !(
            course.feeMin <= (opt.max ?? Infinity) &&
            course.feeMax >= (opt.min ?? 0)
          )
        )
          return false;
      }
    ...
    if (answers.salary) {
      ...
      if (
        ...
        course.avgSalaryMax !== undefined
      ) {
        if (
          !(
            course.avgSalaryMin <= (opt.max ?? Infinity) &&
            course.avgSalaryMax >= (opt.min ?? 0)
          )
        )
          return false;
      }
    ...
    if (
      answers.scholarship &&
      answers.scholarship !== "none" &&
      answers.scholarship !== "any"
    ) {
      if (
        course.scholarships &&
        !course.scholarships.includes(answers.scholarship)
      )
        return false;
    }
...
interface UseDegreeFinderFlowReturn {
  flowStep: FlowStep;
  setFlowStep: React.Dispatch<React.SetStateAction<FlowStep>>;
  stepNumber: number;
